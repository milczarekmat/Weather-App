...
object ForecastRepository {
    ...
    private var currentForecast: MutableLiveData<ForecastModel?>? =
        MutableLiveData<ForecastModel?>()

    fun getCurrentForecast(context: Context): ForecastModel? {
        if (!Network.isNetworkAvailable(context)) {
            val file = File(context.filesDir, "forecast_data")
            if (file.exists()) {
                val forecastData = file.readText()

                return gson.fromJson(forecastData, ForecastModel::class.java)
            }
        }
            ...
            client.newCall(request).enqueue(object : okhttp3.Callback {
                ...
                override fun onResponse(call: okhttp3.Call, response: okhttp3.Response) {
                    if (response.isSuccessful) {
                        ...
                        Log.i(
                            "ForecastRepository",
                            "getCurrentWeather onResponse: ${response.message}"
                        )
                        callback(null)
                    }
                }
